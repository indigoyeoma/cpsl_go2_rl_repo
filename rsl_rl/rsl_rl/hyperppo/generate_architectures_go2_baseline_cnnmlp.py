...
def main(args):
    ...
    for cnn_layers in args.cnn_layer_options:
        for mlp_layers in args.mlp_layer_options:
            # Force exact optimal kernel sequence (all 3x3)
            if cnn_layers == 4:
                kernel_configs = [(3, 3, 3, 3)]
            else:
                kernel_configs = list(product(args.cnn_kernel_options, repeat=cnn_layers))
            
            # Force exact optimal sequence instead of combinations
            if cnn_layers == 4:
                increasing_channel_configs = [(32, 64, 128, 256)]
            else:
            ...
            # Force exact optimal MLP sequence  
            if mlp_layers == 2:
                mlp_layer_configs = [(512, 256)]
            else:
                ...
                for kernels_combo in kernel_configs:
                    for mlp_combo in mlp_layer_configs:
                        ...
                        for i in range(cnn_layers):
                            # Optimal CNN architecture parameters with 3x3 kernels
                            if cnn_layers == 4:  # Optimal 4-layer architecture
                                if i == 0:  # Layer 1: 3x3 kernel, stride=4, pad=1 (84→21)
                                    stride, padding = 4, 1
                                elif i == 1:  # Layer 2: 3x3 kernel, stride=2, pad=1 (21→11)
                                    stride, padding = 2, 1
                                elif i == 2:  # Layer 3: 3x3 kernel, stride=2, pad=1 (11→6)
                                    stride, padding = 2, 1
                                else:  # Layer 4: 3x3 kernel, stride=2, pad=1 (6→3)
                                    stride, padding = 2, 1
                            else:  # Default for other configurations
                                stride, padding = 2, 1
                        ...
                        # CNN branch: flatten → 256 features
                        cnn_mlp_config = [256]
                        # State branch: 48 → 256 → 256 features  
                        state_mlp_config = [256, 256]
                        # Combined policy: 512 (256+256) → mlp_combo → 12
                        first_mlp_input = 256 + 256  # CNN features + State features
    ...
    data = {
        "metadata": {
            ...
            "arch_descriptor_config": {
                ...
                "description": "Format: [ch1,k1,ch2,k2,ch3,k3,ch4,k4,mlp_input,mlp1,mlp2,mlp3] - mlp_input=512(256+256)"
            }
        },
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="configs_go2/architecture_go2_baseline.json", help="Output JSON file path")
