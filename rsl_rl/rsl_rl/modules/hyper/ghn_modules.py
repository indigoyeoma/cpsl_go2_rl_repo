...
from rsl_rl.modules.hyper.utils import capacity, default_device
...
class MLP_GHN(nn.Module):
    ...
    def forward(self, nets_torch, shape_ind = None, return_embeddings=False, predict_class_layers=True, bn_train=True):
        ...
        # Create embeddings for mapped parameters - fix the index mismatch
        # The issue is that params_map keys don't match the embedding indices
        if shape_ind is not None:
            if len(shape_ind) < total_mapped_params:
                # Expand shape indices to match total mapped parameters
                expanded_shape_ind = shape_ind[0:1].expand(total_mapped_params, -1)
                x_before_gnn = self.shape_enc3(expanded_shape_ind)
            else:
                x_before_gnn = self.shape_enc3(shape_ind[:total_mapped_params])
        else:
            raise ValueError("shape_ind is required")
        ...
        for key, inds in param_groups.items():
            ...
            if key == 'cls_w':
                # ALL weights (2D MLP + 4D Conv) - decoder handles tiling/slicing  
                # Use proper max_shape from decoder config for kernel size
