...
class LeggedRobotCfg(BaseConfig):
    ...
    class depth:
        ...
        position = [0.27, 0, 0.035]  # camera position relative to base
        position_rand = 0.01
        angle = [0, 0]  # camera angle [min, max] (positive pitch down)
        original = (106, 60)  # original camera image size
        resized = (87, 58)    # resized image size for processing
        horizontal_fov = 87   # field of view in degrees
        update_interval = 5   # update depth every N steps
        buffer_len = 2        # temporal buffer length
        near_clip = 0.1       # near clipping distance
        far_clip = 2.0        # far clipping distance  
        dis_noise = 0.05      # depth noise magnitude
